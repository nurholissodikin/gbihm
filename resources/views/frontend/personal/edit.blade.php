        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Edit Data - &nbsp;{{$personal->namalengkap}}</h3>
  ...
  function edit_jemaat(jenis,id){
    if (jenis == 'diserahkan') {
      ...
      $.get("{{url('baptisan/get/')}}"+"/"+id,function(item){
        ...
        $('#d_foto1_brk').val(item.d_foto1);
        $('#d_foto2_brk').val(item.d_foto2);
...
// $("#formjabpel").on("submit",function(e) {
//    e.preventDefault();
//       var formpen = e.target;
//       var formdata = new FormData(formpen);
//       $.ajax({
...
function edit_jabpel(id){
  $('#tjp_f').click();
   $('#formjabpel').attr('action','{{url('jabpel/update')}}'+'/'+id);
