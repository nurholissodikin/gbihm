            ...
            <!-- /.box-header -->
             <div class="box-body">
              <div class="box-header with-border">
               <h3 class="box-title">Tambah Kehadiran BP2N</h3>
              </div>
              <br>
             <form  action="{{route('bp2nkehadiran.update',$kehadiran->id)}}" enctype="multipart/form-data" method="post">
                  ...
                  <div class="form-group">
                    <label>Peserta</label>
                      <select class="form-control select2" name="peserta" id="id_peserta" style="width: 100%;" >
                        ...
                        <option value="{{$data->id}}" <?php echo ($kehadiran->idpeserta == $data->id) ? "selected" : "" ?>>{{$data->idpria}} | {{$data->pria['namalengkap']}}</option>
                        @endforeach
                      </select>
                  </div>                  
                </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    $("#id_peserta").select2().on("change", function (e) {
      ...
      $.get("{{url('bp2npeserta/get_peserta')}}"+"/"+id,function(value){
