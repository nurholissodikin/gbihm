            ...
            <div class="box-body">
              <div class="table-responsive">       
                <table width="100%" class="table table-bordered table-hover data-table"   cellspacing="0">
                  ...
                  <tbody id="kom_data">
                  	<form action="{{url('pengerja_approve/gembalarayon/all')}}" method="post" id="form-kirim">  
                  	{{csrf_field()}}
	                    @php $no = 1 @endphp
	                    ...
	                    <tr class="item{{$item->id}}">
	                      <?php 
		                      ...
		                      if($item->approve_gembala_rayon != null){
		                      	$kirim="disable";
		                      }
		                      else {
		                      	$kirim = "";
		                      ...
		                      if($item->approve_gembala_rayon=='0'){
		                      	$approvea="Waiting Acc Gembala Rayon";
		                      	$coloraa="bg-maroon";
		                      }else if($item->approve_gembala_rayon=='1'){
		                      	$approvea="Di Acc by Gembala Rayon";
		                      	$coloraa="bg-green";
		                      }
		                      else{
		                      	$approvea="";
	                      ...
	                      ?>	
	                      <td><input type="checkbox" name="kirima[]" class="selected-all-cool" value="{{$item->id}}" @if($approve =='Waiting Acc'){ disabled  } @elseif($kirim =='disable') disabled  checked @endif></td>
	                      <td>{{$no}}</td>
	                      <td><a href="{{ url('mdpj/pengerja/detail',$item->id)}}" type="submit" class="btn btn-block-small btn-info btn-xs">{{$item->idpersonal}}</a></td>
